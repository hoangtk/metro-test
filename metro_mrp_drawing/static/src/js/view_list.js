...
instance.web.ListView = instance.web.View.extend( /** @lends instance.web.ListView# */ {
    ...
    do_delete: function (ids) {
		var self = this;
		//+++ HoangTK - 03/03/2016 : Display confirm with part name
		var $message = _t("Do you really want to remove these records?");
		if (this.dataset.model == 'drawing.order.line')
		{
			var $part_names = new Array();

			_(ids).each(function (id) {
                record = self.records.get(id);
				$part_names.push(record.attributes.product_id[1]);
            });
			var $all_part_name = $part_names.join();
			$message = _.str.sprintf( _t("Do you really want to remove Part # %s ?"), $all_part_name);

		}
        //if (!(ids.length && confirm(_t("Do you really want to remove these records?")))) {
		if (!(ids.length && confirm($message))) {
            return;
        }
		//--- HoangTK - 03/03/2016 : Display confirm with part name
    ...
    init: function (group, opts) {
        ...
        this.$current = $('<tbody>')
            ...
            .delegate('td.oe_list_record_delete button', 'click', function (e) {
				...
				//+++ HoangTK - 03/03/2016 : Display confirm with part name
				var $message = _t("Do you really want to remove these records?");
				if (self.dataset.model == 'drawing.order.line')
				{
					record = self.records.get(self.row_id($row));
					$message = _.str.sprintf( _t("Do you really want to remove Part # %s ?"), record.attributes.product_id[1]);
				}
				//-- HoangTK - 03/03/2016 : Display confirm with part name
                if (!confirm($message)) {
                	 return;
